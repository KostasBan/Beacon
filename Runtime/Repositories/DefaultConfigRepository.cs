...
namespace KostasBan.Beacon.Repositories
{
    public sealed class DefaultConfigRepository : IConfigRepository
    {
        ...
        public async Task<RefreshResult> RefreshAsync(CancellationToken ct)
        {
            ...
            if (HasNoMaterialChange(fetchResult.Bytes))
            {
                return new RefreshResult(false, null);
            }
        ...
        private bool HasNoMaterialChange(byte[] bytes)
        {
            if (_snapshot.RawBytes == null)
            {
                return false;
            ...
            return _snapshot.RawBytes.SequenceEqual(bytes);
        }
    }
}
